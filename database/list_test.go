...
func TestPush(t *testing.T) {
	...
	for i := 0; i < size; i++ {
		...
		values[i] = []byte(value)
		result := testDB.execWithLock(utils.ToCmdLine("rpush", key, value))
		if intResult, _ := result.(*reply.IntReply); intResult.Code() != int64(i+1) {
			t.Errorf("expected %d, actually %d", i+1, intResult.Code())
		}
	}
	actual := testDB.execWithLock(utils.ToCmdLine("lrange", key, "0", "-1"))
	...
	result := testDB.execWithLock(utils.ToCmdLine2("rpush", args...))
	...
	for i := 0; i < size; i++ {
		...
		result = testDB.execWithLock(utils.ToCmdLine("lpush", key, value))
		if intResult, _ := result.(*reply.IntReply); intResult.Code() != int64(i+1) {
			t.Errorf("expected %d, actually %d", i+1, intResult.Code())
		}
	}
	...
	result = testDB.execWithLock(utils.ToCmdLine2("lpush", args...))
	if intResult, _ := result.(*reply.IntReply); intResult.Code() != int64(size) {
		t.Errorf("expected %d, actually %d", size, intResult.Code())
	}
	actual = testDB.execWithLock(utils.ToCmdLine("lrange", key, "0", "-1"))
...
func TestLRange(t *testing.T) {
	...
	for i := 0; i < size; i++ {
		value := utils.RandString(10)
		testDB.execWithLock(utils.ToCmdLine("rpush", key, value))
		values[i] = []byte(value)
	}

	start := "0"
	end := "9"
	actual := testDB.execWithLock(utils.ToCmdLine("lrange", key, start, end))
...
func TestLPop(t *testing.T) {
	...
	testDB.execWithLock(utils.ToCmdLine2("rpush", values...))
	size := len(values) - 1

	for i := 0; i < size; i++ {
		result := testDB.execWithLock(utils.ToCmdLine("lpop", key))
	...
	result := testDB.execWithLock(utils.ToCmdLine("rpop", key))
	expected := reply.NewNullBulkReply()
	if !utils.BytesEquals(result.Bytes(), expected.Bytes()) {
		t.Errorf("expected %s, actually %s", string(expected.Bytes()), string(result.Bytes()))
	}*/
	re := execLPop(testDB, utils.ToCmdLine(enum.LPOP.String(), "list"))
